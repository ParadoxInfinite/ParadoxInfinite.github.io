...
$(document).keypress(function (e) {
  ...
  if (e.keyCode == 70 || e.keyCode == 102) {
    $("#Home,#Ski,#Work").removeClass("active");
    $("#Edu").addClass("active");
    var education = $("#education").position().top;
    var distance = Math.abs(pos + education);
    if (distance != 0) {
      $("html, body").animate({ scrollTop: education }, 800);
    }
  }
  if (e.keyCode == 87 || e.keyCode == 119) {
    $("#Home,#Edu,#Ski").removeClass("active");
    $("#Work").addClass("active");
    var skills = $("#work").position().top;
    var distance = Math.abs(pos + skills);
    if (distance != 0) {
      $("html, body").animate({ scrollTop: skills }, 800);
    }
  }
  if (e.keyCode == 81 || e.keyCode == 113) {
    $("#Home,#Edu,#Work").removeClass("active");
    $("#Ski").addClass("active");
  ...
  if (e.keyCode == 82 || e.keyCode == 114) {
    $("#Edu,#Ski,#Work").removeClass("active");
    $("#Home").addClass("active");
    var home = $("#mainpage").position().top;
    var distance = Math.abs(pos + home);
    if (distance != 0) {
      $("html, body").animate({ scrollTop: home }, 800);
    }
  }
});
